...
# upsample positive examples in training data
train_data_positive = train_data.loc[train_data["label"]]
train_data_negative = train_data.loc[train_data["label"]==0]
train_data_positive = train_data_positive.loc[train_data_positive.index.repeat(8)]
...
# save the model
trainer.save_model(f"./models/saved_model_upsampling/")

############

# load the model
model = DebertaForSequenceClassification.from_pretrained(f"./models/saved_model_upsampling/")
model.to(device)
...
with open(f"val_eval_with_upsampling.txt", "w") as file:
